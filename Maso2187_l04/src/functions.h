...
/**
 * Reads consecutive lines of text from fp until either the end of file is
 * reached or the end of the array of pointers is reached. Each input line is
 * stored in dynamic storage and it is accessed/referenced by the strings array.
 ...
 * @return - number of lines read; MALLOC_FAILED: if malloc fails;
 * TOO_MANY_LINES: if size of strings exceeded
 */
int read_strings(strings_array *data, FILE *fp);

/**
 * Traverses every string in strings and prints the strings that contain substr.
 * Case must match.
 *
...
/**
 * Traverses every string in strings and copies all strings shorter than length
 * to the short_lines file, and allstrings equal to or longer than length to the
 * long_lines file.
 *
 * @param data - the strings_array struct to traverse
