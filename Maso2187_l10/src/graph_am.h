...
/**
 * Adds a vertice to source. Fails if pair values invalid or vertice already in
 * source. Pairs containing row < 0 or row >= size are ignored; Pairs containing
 * col < 0 or col >= size are ignored; Pairs containing row == col are a loop
 * and are given a value of 2; All other pairs are given a value of 1 in
 * [row][col] and [col][row].
 *
 * @param source - pointer to a graph
...
/**
 * Removes an vertice from source. Fails if pair values invalid or vertice not
 * in source. Pairs containing row < 0 or row >= size are ignored; Pairs
 * containing col < 0 or col >= size are ignored;
 *
 * @param source - pointer to a graph
