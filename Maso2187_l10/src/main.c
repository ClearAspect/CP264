...
void test_graph(void) {
	...
	printf(SEP);
	printf("Create my adjacency matrix graph:\n");
	graph_am *source1 = graph_am_create(3, pairs1, 3);
	graph_am_print(source1);
	printf(SEP);

	for (int i = 0; i < 3; i++) {
		int vertices[3];
		int count = 0;
		graph_am_breadth_traversal(source1, 0, vertices, &count);
		printf("%6d ", i);
	...
	for (int i = 0; i < 3; i++) {
		int vertices[3];
		int count = 0;
		graph_am_depth_traversal(source1, 0, vertices, &count);
		printf("%6d ", i);
		print_array(vertices, count);
		printf("\n");
	}
