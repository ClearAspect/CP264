...
/* Define structure NODE as linked list node to represent hash data the same
 * hashed value. DATA data - data field in node structure next - pointer to the
 * next node.
 */
...
/* Hash function that hash key string to an integer of modular of hash table
 * size
 ...
 * @return - if the named data exists, delete it and return 1; otherwise return
 * 0.
 */
