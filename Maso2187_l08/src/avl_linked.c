...
static void avl_rebalance(avl_node **node) {
	...
	if (balance > 1) {
		...
		// Left-Left case (or converted Left-Right)
	...
	// Right subtree is heavier
		...
		// Right-Right case (or converted Right-Left)
...
static bool avl_insert_aux(avl_linked *source, avl_node **node,
	...
	if (*node == NULL) {
		// Base case: empty tree or leaf node location found
		*node = avl_node_initialize(item);
		...
		// If comp == 0, item already exists, don't insert
...
static bool avl_remove_aux(avl_linked *source, avl_node **node,
	...
	if (comp < 0) {
		...
		removed = avl_remove_aux(source, &(*node)->left, key, item);
	} else if (comp > 0) {
		// Key is larger, go right
		removed = avl_remove_aux(source, &(*node)->right, key, item);
	} else {
		// Key found - copy data to output parameter
		...
		// Case 4: Node with two children
			...
			removed = avl_remove_aux(source, &(*node)->right, successor->item, item);
		}
	...
	// If we removed a node, update height and rebalance
...
static void avl_copy_aux(avl_node **target_node, const avl_node *source_node) {
	if (source_node == NULL) {
		...
		*target_node = malloc(sizeof **target_node);
		(*target_node)->item = malloc(sizeof *(*target_node)->item);
		data_copy((*target_node)->item, source_node->item);
		(*target_node)->height = source_node->height;

		// Recursively copy left and right subtrees
		avl_copy_aux(&(*target_node)->left, source_node->left);
		avl_copy_aux(&(*target_node)->right, source_node->right);
	}
	return;
}
...
static int avl_leaf_count_aux(const avl_node *node) {
	...
	return avl_leaf_count_aux(node->left) + avl_leaf_count_aux(node->right);
}
...
static int avl_one_child_count_aux(const avl_node *node) {
	...
	return count + avl_one_child_count_aux(node->left) +
...
static int avl_two_child_count_aux(const avl_node *node) {
	...
	// Add counts from left and right subtrees
	return count + avl_two_child_count_aux(node->left) +
...
static bool avl_equals_aux(const avl_node *target_node,
	...
	if (target_node->height != source_node->height) {
		return false;
	}

	// Recursively check if the left and right subtrees are equal
	return avl_equals_aux(target_node->left, source_node->left) &&
